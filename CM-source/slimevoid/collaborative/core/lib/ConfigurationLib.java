...
public class ConfigurationLib {
    ...
    public static void CommonConfig(File configFile) {
        ...
        blockCollaborativeBaseID = configuration.get(Configuration.CATEGORY_BLOCK,
                                                     "collaborativeBaseID",
                                                     1890).getInt();
        ...
        itemPlanBlankID = configuration.get(Configuration.CATEGORY_BLOCK,
                                            "planBlankID",
                                            18900).getInt();
        itemPlanFullID = configuration.get(Configuration.CATEGORY_BLOCK,
                                           "planFullID",
                                           18901).getInt();
        playerInventoryLocked = configuration.get(Configuration.CATEGORY_GENERAL,
                                                  "playerInventoryLocked",
                                                  true).getBoolean(true);
        debug = configuration.get(Configuration.CATEGORY_GENERAL,
                                  "debug",
    ...
    public static void updatePlayersInventoryLocked(EntityPlayer entityplayer, boolean inventoryMode) {
        ...
        playersInventoryLocked.put(entityplayer,
                                   inventoryMode);
    }

    public static boolean isPlayerInventoryLocked(EntityPlayer entityplayer) {
        boolean flag = !playersInventoryLocked.containsKey(entityplayer)
                       || playersInventoryLocked.get(entityplayer);
        return flag;
    }
